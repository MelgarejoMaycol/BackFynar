...
import type { CreateLoanInput, ListLoansInput, PaymentInput, SimulationInput, UpdateLoanInput } from "./lending.schemas.js";
...
export class LendingService {
  ...
  simulate(input: SimulationInput) {
    ...
    const first = input.firstPaymentDate ? new Date(`${input.firstPaymentDate}T00:00:00.000Z`) : null;
    return {
      ...summary,
  ...
  private async refreshStatuses(tx: Prisma.TransactionClient, workspaceId: string, loanId?: string) {
  ...
  async list(workspaceId: string, input: ListLoansInput = {}): Promise<Array<Record<string, unknown>>> {
    ...
    return rows.map((row) => ({ ...row,
      ...
      estimatedEndDate: row.estimatedEndDate instanceof Date ? dateOnly(row.estimatedEndDate) : row.estimatedEndDate,
    }));
  }
  ...
  async summary(workspaceId: string) {
    ...
    return {
      ...
      upcoming: upcoming.map((row) => ({ ...row, dueDate: row.dueDate instanceof Date ? dateOnly(row.dueDate) : row.dueDate })),
    };
  }
  ...
  async get(
    ...
    `);
    const loan = loans[0]!;
    for (const key of ["disbursementDate", "firstPaymentDate", "nextDueDate", "estimatedEndDate"] as const) {
      if (loan[key] instanceof Date) loan[key] = dateOnly(loan[key] as Date);
    }
    return { ...loan, installments: installments.map((row) => ({ ...row, dueDate: row.dueDate instanceof Date ? dateOnly(row.dueDate) : row.dueDate })), payments };
  }
  ...
  async create(workspaceId: string, userId: string, input: CreateLoanInput) {
    ...
    const loanId = await this.db.$transaction(async (tx) => {
      const person = await tx.financialPerson.findFirst({ where: { id: input.personId, workspaceId, isActive: true, archivedAt: null } });
      if (!person) throw new NotFoundError("Persona financiera no encontrada o archivada");
      const source = input.sourceAccountId ? await tx.financialAccount.findFirst({ where: { id: input.sourceAccountId, workspaceId, nature: "ASSET", isActive: true, deletedAt: null } }) : null;
      if (input.sourceAccountId && !source) throw new NotFoundError("Cuenta de origen no encontrada o archivada");
      if (source?.currency !== undefined && source.currency !== input.currency) throw new ValidationError("La moneda del préstamo y la cuenta de origen deben coincidir");
      if (source) {
        await tx.$queryRaw(Prisma.sql`SELECT id FROM financial_accounts WHERE workspace_id=${workspaceId}::uuid AND id=${source.id}::uuid FOR UPDATE`);
        const current = await tx.financialAccount.findUnique({ where: { id: source.id } });
        if (!current || current.currentBalance.lt(principal)) throw new ConflictError("Fondos insuficientes", "No tienes saldo suficiente para desembolsar este préstamo.");
      }
      const receivable = await tx.financialAccount.create({ data: {
        workspaceId, name: `Préstamo a ${person.name} · ${randomUUID().slice(0, 8)}`, type: "LOAN", nature: "ASSET",
        currency: input.currency, openingBalance: source ? 0 : principal, currentBalance: source ? 0 : principal,
        includeInNetWorth: true, icon: "HandCoins",
      }});
      if (source) {
        await tx.financialAccount.update({ where: { id: source.id }, data: { currentBalance: { decrement: principal } } });
        await tx.financialAccount.update({ where: { id: receivable.id }, data: { currentBalance: { increment: principal } } });
        await tx.transaction.create({ data: {
          workspaceId, createdBy: userId, type: "TRANSFER", status: "CONFIRMED", amount: principal,
          currency: input.currency, accountId: source.id, destinationAccountId: receivable.id,
      ...
      const created = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`
        INSERT INTO issued_loans (workspace_id,person_id,receivable_account_id,source_account_id,currency,
        ...
        VALUES (${workspaceId}::uuid,${person.id}::uuid,${receivable.id}::uuid,${source?.id ?? null}::uuid,
      ...
      await tx.auditLog.create({ data: { workspaceId, userId, entityType: "ISSUED_LOAN", entityId: id, action: "CREATE", newData: json({ personId: person.id, principal: input.principal, historical: !source }) } });
      return id;
  ...
  async update(workspaceId: string, userId: string, loanId: string, input: UpdateLoanInput) {
    if (input.personId) {
      const person = await this.db.financialPerson.findFirst({ where: { id: input.personId, workspaceId, isActive: true, archivedAt: null } });
      if (!person) throw new NotFoundError("Persona financiera no encontrada o archivada");
    }
    ...
    const changed = await this.db.$transaction(async (tx) => {
      const count = await tx.$executeRaw(Prisma.sql`UPDATE issued_loans SET ${Prisma.join(changes, ",")},updated_at=now() WHERE workspace_id=${workspaceId}::uuid AND id=${loanId}::uuid AND archived_at IS NULL`);
      if (count) await tx.auditLog.create({ data: { workspaceId, userId, entityType: "ISSUED_LOAN", entityId: loanId, action: "UPDATE", newData: json(input) } });
      return count;
    });
  ...
  async pay(
    ...
    input: PaymentInput,
  ): Promise<PublicLoan & { paymentId: string | null; idempotent: boolean }> {
    ...
    await this.db.$transaction(async (tx) => {
      ...
      if (existing[0]) { paymentId = existing[0].id; idempotent = true; return; }
      ...
      if (loan.status === "PAID") throw new ConflictError("Préstamo pagado", "Este préstamo ya no tiene cobros pendientes.");
      const installments = await tx.$queryRaw<InstallmentLock[]>(Prisma.sql`
        SELECT id,due_date,principal_amount,principal_paid,interest_amount,interest_paid,total_amount,total_paid
        FROM issued_loan_installments WHERE workspace_id=${workspaceId}::uuid AND loan_id=${loanId}::uuid AND id=${installmentId}::uuid FOR UPDATE
      `);
      const installment = installments[0];
      if (!installment) throw new NotFoundError("Cuota no encontrada");
      const earlier = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`
        SELECT id FROM issued_loan_installments WHERE workspace_id=${workspaceId}::uuid AND loan_id=${loanId}::uuid
          AND installment_number < (SELECT installment_number FROM issued_loan_installments WHERE id=${installmentId}::uuid)
          AND total_paid < total_amount LIMIT 1
      `);
      if (earlier[0]) throw new ConflictError("Cuota anterior pendiente", "Los cobros deben aplicarse en el orden del plan de pagos.");
      ...
      await tx.$queryRaw(Prisma.sql`SELECT id FROM financial_accounts WHERE workspace_id=${workspaceId}::uuid AND id IN (${loan.receivable_account_id}::uuid,${account.id}::uuid) ORDER BY id FOR UPDATE`);
      const remaining = decimal(installment.total_amount).minus(installment.total_paid);
      if (amount.gt(remaining)) throw new ConflictError("Cobro superior a la cuota pendiente", "El cobro no puede superar el saldo pendiente de esta cuota.");
      const interestRemaining = decimal(installment.interest_amount).minus(installment.interest_paid);
      const interestPart = Prisma.Decimal.min(amount, Prisma.Decimal.max(0, interestRemaining));
      ...
      if (!receivable || receivable.currentBalance.lt(principalPart)) throw new ConflictError("Saldo por cobrar inconsistente", "El saldo contable del préstamo no permite aplicar este cobro.");
      ...
      if (principalPart.gt(0)) {
        ...
        principalTransactionId = (await tx.transaction.create({ data: {
          workspaceId, createdBy: userId, type: "TRANSFER", status: "CONFIRMED", amount: principalPart,
          currency: loan.currency, accountId: receivable.id, destinationAccountId: account.id, occurredAt,
          description: `Recuperación de capital · ${loan.person_name}`, metadata: { lending: true, loanId, role: "PRINCIPAL_COLLECTION" },
        }})).id;
      }
      if (interestPart.gt(0)) {
        await tx.financialAccount.update({ where: { id: account.id }, data: { currentBalance: { increment: interestPart } } });
        interestTransactionId = (await tx.transaction.create({ data: {
          workspaceId, createdBy: userId, type: "INCOME", status: "CONFIRMED", amount: interestPart,
          currency: loan.currency, accountId: account.id, occurredAt,
          description: `Interés recibido · ${loan.person_name}`, notes: input.notes ?? null,
          metadata: { lending: true, loanId, role: "INTEREST_INCOME" },
        }})).id;
      }
      const totalPaid = decimal(installment.total_paid).plus(amount);
      const paid = totalPaid.gte(installment.total_amount);
      await tx.$executeRaw(Prisma.sql`
        UPDATE issued_loan_installments SET principal_paid=principal_paid+${principalPart},interest_paid=interest_paid+${interestPart},
          total_paid=total_paid+${amount},status=${paid ? "PAID" : "PARTIAL"},paid_at=${paid ? occurredAt : null},updated_at=now()
        WHERE workspace_id=${workspaceId}::uuid AND loan_id=${loanId}::uuid AND id=${installmentId}::uuid
      `);
      const inserted = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`
        ...
        VALUES (${workspaceId}::uuid,${loanId}::uuid,${installmentId}::uuid,${account.id}::uuid,
          ${principalTransactionId}::uuid,${interestTransactionId}::uuid,${amount},${principalPart},${interestPart},
          ${occurredAt},${input.notes ?? null},${input.idempotencyKey},${userId}::uuid) RETURNING id
      `);
      paymentId = inserted[0]!.id;
      await tx.$executeRaw(Prisma.sql`UPDATE issued_loans SET current_principal=current_principal-${principalPart},principal_received=principal_received+${principalPart},interest_received=interest_received+${interestPart},updated_at=now() WHERE workspace_id=${workspaceId}::uuid AND id=${loanId}::uuid`);
      await this.refreshStatuses(tx, workspaceId, loanId);
      await tx.auditLog.create({ data: { workspaceId, userId, entityType: "ISSUED_LOAN_PAYMENT", entityId: paymentId, action: "CREATE", newData: json({ loanId, installmentId, amount: input.amount, principal: principalPart.toString(), interest: interestPart.toString() }) } });
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
    return { ...(await this.get(workspaceId, loanId)), paymentId, idempotent };
  }

  async reverse(workspaceId: string, userId: string, loanId: string, paymentId: string, reason: string) {
    await this.db.$transaction(async (tx) => {
      ...
      const loan = loans[0];
      if (!loan) throw new NotFoundError("Préstamo no encontrado");
      await tx.$queryRaw(Prisma.sql`SELECT id FROM financial_accounts WHERE workspace_id=${workspaceId}::uuid AND id IN (${loan.receivable_account_id}::uuid,${payment.receiving_account_id}::uuid) ORDER BY id FOR UPDATE`);
      const receiving = await tx.financialAccount.findFirst({ where: { id: payment.receiving_account_id, workspaceId } });
      if (!receiving || receiving.currentBalance.lt(payment.total_received)) throw new ConflictError("Saldo insuficiente para revertir", "La cuenta receptora ya no tiene saldo suficiente para revertir este cobro de forma íntegra.");
      if (payment.principal_received.gt(0)) await tx.financialAccount.update({ where: { id: loan.receivable_account_id }, data: { currentBalance: { increment: payment.principal_received } } });
      await tx.financialAccount.update({ where: { id: receiving.id }, data: { currentBalance: { decrement: payment.total_received } } });
      const transactionIds = [payment.principal_transaction_id, payment.interest_transaction_id].filter((id): id is string => Boolean(id));
      if (transactionIds.length) await tx.transaction.updateMany({ where: { workspaceId, id: { in: transactionIds }, deletedAt: null }, data: { status: "CANCELLED", deletedAt: new Date(), updatedAt: new Date() } });
      await tx.$executeRaw(Prisma.sql`
      ...
      `);
      await tx.$executeRaw(Prisma.sql`UPDATE issued_loan_payments SET reversed_at=now(),reversed_by=${userId}::uuid,reversal_reason=${reason} WHERE workspace_id=${workspaceId}::uuid AND id=${paymentId}::uuid`);
      await this.refreshStatuses(tx, workspaceId, loanId);
      await tx.auditLog.create({ data: { workspaceId, userId, entityType: "ISSUED_LOAN_PAYMENT", entityId: paymentId, action: "REVERSE", newData: json({ reason }) } });
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
    return this.get(workspaceId, loanId);
  }
  ...
  async archive(workspaceId: string, userId: string, loanId: string) {
    const changed = await this.db.$transaction(async (tx) => {
      const loans = await tx.$queryRaw<Array<{ current_principal: Prisma.Decimal; status: string }>>(Prisma.sql`SELECT current_principal,status FROM issued_loans WHERE workspace_id=${workspaceId}::uuid AND id=${loanId}::uuid AND archived_at IS NULL FOR UPDATE`);
      const loan = loans[0];
      if (!loan) throw new NotFoundError("Préstamo no encontrado");
      if (loan.current_principal.gt(0) || loan.status !== "PAID") throw new ConflictError("Préstamo con saldo pendiente", "Solo puedes archivar un préstamo completamente pagado.");
      await tx.$executeRaw(Prisma.sql`UPDATE issued_loans SET status='ARCHIVED',archived_at=now(),updated_at=now() WHERE workspace_id=${workspaceId}::uuid AND id=${loanId}::uuid`);
      await tx.auditLog.create({ data: { workspaceId, userId, entityType: "ISSUED_LOAN", entityId: loanId, action: "ARCHIVE" } });
      return true;
    });
    return { id: loanId, archived: changed };
  }
}
