...
export const collect = route(async (request, response) => response.status(201).json({ success: true, data: await service.pay(workspace(request), request.auth!.userId, parse(uuid, request.params.loanId), null, parse(paymentSchema, request.body)) }));
