...
function impactCopy(level: ImpactLevel) {
  if (level === "CRITICAL") return {
    headline: "Esta compra podría dejarte sin margen para cubrir el periodo",
    explanation: "La simulación lleva tu saldo proyectado o tu punto de menor liquidez por debajo de cero.",
  };
  if (level === "HIGH") return {
    headline: "Puedes asumirla, pero quedarías con muy poco margen",
    explanation: "La compra consumiría una parte importante del saldo que hoy proyectas conservar al cierre.",
  };
  if (level === "MODERATE") return {
    headline: "La compra es posible, pero sí cambia tu margen del periodo",
    explanation: "Seguirías con saldo proyectado positivo, aunque con menos espacio para imprevistos.",
  };
  return {
    headline: "La compra tendría un impacto bajo en tu proyección actual",
    explanation: "Con los compromisos y supuestos conocidos, conservarías la mayor parte de tu margen proyectado.",
  };
}
...
export class SimulationsService {
  ...
  async purchase(
    ...
    if (input.accountId && !account) {
      throw new AppError("Cuenta no encontrada para la simulación", {
        status: 404,
        code: "SIMULATION_ACCOUNT_NOT_FOUND",
        publicMessage: "La cuenta seleccionada ya no está disponible",
      });
    }
    if (input.paymentMethod === "CASH" && account?.nature !== "ASSET") {
      throw new AppError("La compra de contado requiere una cuenta de activo", {
        status: 400,
        code: "SIMULATION_INVALID_CASH_ACCOUNT",
        publicMessage: "Selecciona una cuenta con dinero disponible",
      });
    }
    if (input.paymentMethod === "CREDIT_CARD" && account?.type !== "CREDIT_CARD") {
      throw new AppError("La simulación con tarjeta requiere una tarjeta de crédito", {
        status: 400,
        code: "SIMULATION_INVALID_CARD",
        publicMessage: "Selecciona una tarjeta de crédito válida",
      });
    }

    const rate = input.monthlyRate ?? 0;
    const monthlyPayment = input.paymentMethod === "CASH"
      ? input.amount
      : fixedPayment(input.amount, input.installments, rate);
    ...
    // En contado el desembolso completo afecta el periodo. En financiación se añade la primera cuota
    // como compromiso conservador; no se crea ningún movimiento ni obligación real.
    ...
    return {
      ...
      before: {
        currentAvailable: primary.currentAvailable,
        projectedClosingBalance: primary.projectedClosingBalance,
        lowestProjectedBalance: primary.lowestProjectedBalance,
        knownCommitments: primary.knownCommitments,
      },
      after: {
        projectedClosingBalance: money(afterClosing),
        lowestProjectedBalance: { ...primary.lowestProjectedBalance, amount: money(lowestAfter) },
        addedCommitmentThisPeriod: money(periodImpact),
        selectedAccountAfter: accountAfter === null ? null : money(accountAfter),
      },
      ...
      assumptions: [
        ...
        input.paymentMethod === "CASH"
          ? "La compra de contado se descuenta completa del periodo actual."
          : "Para medir el impacto inmediato se agrega una cuota al periodo actual de forma conservadora.",
